...
function Main() {
  ...
  const btnClass = cn("navigation-menu", {
    "navigation-menu-open": isNavigationMenuOpen,
  });

  const conf = useContext(LangContext);

  useEffect(() => {
    getUserInfo({ per_page: 10 }).then((data) => {
      setUsers(data);
  ...
  const handlePageClick = () => {
    getUserInfo({ since: users[users.length - 1].id, per_page: 10 }).then(
      (data) => {
        setUsers(data);
      }
    );
    ...
    <div className="app-container">
      <header className="app-header">
        <div className="container navigation-container">
          ...
          <nav className={btnClass}>
            <ul className="link header-links">
              {conf.languageChanging.header[conf.language].header_links.map(
                (item, i) => {
                  return (
          ...
          <button
            type="button"
            className="connection-links-toggler"
            onClick={() => {
              setIsNavigationMenuOpen(!isNavigationMenuOpen);
            }}
          >
            <span className="visually-hidden">
              Close / Open navigation window
            </span>
          </button>
        </div>
      </header>
      ...
      <main className="container app-content">
        ...
        <Link to={`/search/${search}`}>
          {conf.languageChanging.header[conf.language].link}
        </Link>
        <div className="two-columns-container">
          <aside className="aside-content">
            <ul className="link main-page-link">
              {conf.languageChanging.header[conf.language].main_page_link.map(
                (item, i) => {
                  return (
                    <li key={i}>
                      <a href="#">{item}</a>
          ...
          <div className="user-list-container">
            <h1 className="main-page-header">
              {conf.languageChanging.header[conf.language].header}
            </h1>
            <List users={users} />
          </div>
        </div>
      ...
      <footer className="container page-footer">
        <div className="logo-footer">Footer-Logo</div>
        <p>Washington str. 32/55</p>
        <div className="footer-connects">
          <ul className="link user-footer-links">
            {conf.languageChanging.footer[conf.language].footer_links.map(
              (item, i) => {
                return (
                  <li key={i}>
                    <a href="#">{item}</a>
        ...
        <div className="link user-links">
          {conf.languageChanging.footer[conf.language].user_footer_links.map(
            (item, i) => {
              return (
                <li key={i}>
                  <a href="#">{item}</a>
